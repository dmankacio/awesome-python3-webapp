...
class ModelMetaClass(type):
    def __new__(cls, name, bases, attrs):
        ...
        log('found model %s (table:%s)' % (name, tableName))
        # 获取所有的Field和主键名:
        mappings = dict()
        fields = []
        primaryKey = None
        for k,v in attrs.items():
            if (isinstance(v, Field)):
                ...
                if v.primary_key:
                    if primaryKey:
                        raise RuntimeError('Duplicated primary key for Field %s' % v)
                    primaryKey = k
                else:
                    fields.append(k)
        if not primaryKey:
...
class Model(dict, metaclass=ModelMetaClass):
    ...
    def getValueOrDefault(self, key):
        #log('getValueOrDefault: %s,%s', (self, key))
    ...
    async def save(self):
        log('save: fields= %s' % self.__fields__)
